// Command schemademo runs this module's examples as programs.
//
...
import (
	...
	"github.com/mbauer83/effect-golang-schema/examples/catalog"
...
func main() {
	workspace, err := os.MkdirTemp("", "schemademo")
	if err != nil {
		fail(err)
	}
...
// runCatalog loads a catalogue, writes it back normalised, and publishes the
// contract that says what it accepts -- one description doing three jobs.
func runCatalog(runtime *effect.Runtime, workspace string) {
	...
	program := catalog.Program(inputPath,
		filepath.Join(workspace, "normalised.json"), contractPath)
...
func fail(err error) {
	fmt.Fprintf(os.Stderr, "schemademo: %v\n", err)
	os.Exit(1)
}
